        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-4 text-xs font-medium ml-auto">
            <span className="hidden sm:inline-block text-emerald-400 font-semibold">
              ● Devis d'ingénierie sous 48h
            </span>
            {isAuthenticated ? (
              user?.role !== 'user' ? (
                <Link
                  to="/admin/dashboard"
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700 text-emerald-300 hover:text-white hover:bg-emerald-900 transition-colors font-semibold"
                >
                ...
                <Link
                  to="/mon-espace"
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-200 hover:text-white hover:bg-slate-700 transition-colors font-semibold"
                >
              ...
              <div className="pt-2 border-t border-slate-100 space-y-1">
                ...
                {domainSubmenu.map((sub) => (
                  <Link
                    key={sub.slug}
                    to={`/domaines/${sub.slug}`}
                    className="block px-3 py-2 text-sm font-medium text-slate-600 hover:text-emerald-800 hover:bg-slate-50 rounded-lg"
                  >
                    <span>{sub.title}</span>
                  </Link>
                ))}
